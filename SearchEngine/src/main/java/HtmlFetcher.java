...
public class HtmlFetcher {
	...
	public static boolean isHtml(Map<String, List<String>> headers) {
		for(String key:headers.keySet()) {
			if(key!=null&&key.equals("Content-Type")) {
				return headers.get(key).get(0).startsWith("text/html");
			}
		}
		return false;
	}
	...
	public static int getStatusCode(Map<String, List<String>> headers) {
		for(Entry<String, List<String>> entry:headers.entrySet()) {
			if(entry.getKey()==null) {
				String[] line=entry.getValue().get(0).split(" ");
	...
	public static String fetch(URL url, int redirects) throws IOException {
		try (
			...
			if(!isHtml(headers)) return null;
			if(getStatusCode(headers)==200) {
				return String.join("\n",HttpsFetcher.getContent(response));
			}
			else if(isRedirect(headers)&&redirects>0) {
				System.out.println(headers.get("Location").get(0)+" Redirects "+redirects);
				//System.out.println(headers.get("Location").toString());
